...
void sort_blob_by_sizeRED(std::vector<cv::KeyPoint> k)
{
	...
	if(k.size() > 0)
	{
		...
		posXRed = big_keypoint_by_size.pt.x;
		posYRed = big_keypoint_by_size.pt.y;
		no_objectRed = false;
	}
	else
	{
		posXRed = drone_center.x;
		posYRed = drone_center.y;
		no_objectRed = true;
...
void move_drone(cv::Mat& image)
{
	...
	if(turning_mode)
	{
		...
		//printf("time %f\n",end_time - start_time);
		cmd_vel_pub_bebop.publish(cmd_vel_bebop);
		if ((end_time-start_time) >= 0.013*angle_to_turn){
			turning_mode = 0;
			printf("HERE\n");	
...
{	
	...
	takeoff_bebop = nh_.advertise<std_msgs::Empty>("/ardrone/takeoff",1000);		//Publish data to the take-off topic
	land_bebop = nh_.advertise<std_msgs::Empty>("/ardrone/land",1000);		//Publish data to the land topic
	cmd_vel_pub_bebop = nh_.advertise<geometry_msgs::Twist>("/cmd_vel",1000);	//Publish data to the movement topic
